...
function createPlayer() {
    ...
    if (playerName.length <= 20 && playerName.length > 0) {
        ...
        // Ajouter le joueur à la leaderboard
        var li = document.createElement('li');
        li.style.order = players.length;

        var crown = document.createElement('span');
        crown.className = 'crown';
        crown.innerHTML = '👑';

        var playerPointLeadBoard = document.createElement('div');
        playerPointLeadBoard.className = 'point';
        playerPointLeadBoard.style.backgroundColor = playerColor;
        playerPointLeadBoard.id = playerName + 'leaderboard';
        playerPointLeadBoard.appendChild(crown);

        var div = document.createElement('div');
        div.className = 'flex';
        div.appendChild(playerPointLeadBoard);
...
function orderLeaderBoard() {
    ...
    // Copy and sort players for the leaderboard
    leaderBoard = JSON.parse(JSON.stringify(players));
    leaderBoard.sort((a, b) => a.CurrentTile - b.CurrentTile);
    leaderBoard.reverse();

    crownedPlayers = [];
    // Get each players leaderboard li and change its order depending on the players place
    leaderBoard.forEach((element, index) => {
        leaderBoardPoint = document.getElementById(element.Name + 'leaderboard');
        parent1 = leaderBoardPoint.parentElement;
        parent2 = parent1.parentElement;
        parent2.style.order = index;

        if (index == 0) {
            elementTile = element.CurrentTile;
            crownedPlayers.push(element);
        } else {
            if (element.CurrentTile == elementTile) {
                crownedPlayers.push(element);
            } else {
                leaderBoardPoint.innerHTML = '';
            }
        }
    });

    crownedPlayers.forEach((element) => {
        leaderBoardPoint = document.getElementById(element.Name + 'leaderboard');
        leaderBoardPoint.innerHTML = '<span class="crown">👑</span>';
    });
